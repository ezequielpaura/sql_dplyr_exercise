...
player_db %>%
  left_join(
    {player_attributes_db %>% 
      group_by(player_api_id) %>%
      summarise(min_rating=min(overall_rating, na.rm=TRUE), max_rating=max(overall_rating, na.rm=TRUE))}, 
    by='player_api_id') %>%
  arrange(min_rating) %>%
  select(player_name, max_rating) %>%
  head(5)
...
match_db %>%
  left_join(country_db, by=c("country_id" = "id"))%>%
    filter(season == '2008/2009', name == "Poland")%>%
    select(home_team_goal,home_team_api_id )%>%
...
match_db %>%
  left_join(country_db, by=c('country_id' = 'id'))%>%
    filter(season == season, name == country)%>%
    select(home_team_goal,home_team_api_id )%>%
    group_by(home_team_api_id)%>%
    summarise(mean=mean(home_team_goal))%>%
    arrange(-mean)%>%
...
match_db %>%
    group_by(away_team_api_id)%>%
 summarise(goals_away=sum(away_team_goal)) %>% 
  left_join(match_db, by = c("away_team_api_id", "home_team_api_id"))
